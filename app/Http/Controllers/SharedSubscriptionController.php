...
class SharedSubscriptionController extends Controller
{
    public function index()
    {
        ...
        // Ensure active subscriptions have invite_code
        ...
        // Subscriptions owned by user with their shared members
        $mySharedSubscriptions = Subscription::where('user_id', $user->id)
            ->with(['shares.friendUser', 'category'])
            ->has('shares')
            ->get();

        // All user's active subscriptions with existing shares count
        ...
        // Subscriptions shared WITH me by friends
    ...
    public function store(Request $request)
    {
        ...
        // Validasi: pastikan benar-benar teman (opsional tapi baiknya ada)
        ...
        // Total members = 1 (Owner) + existing shares + 1 (new member)
        ...
        // Auto recalculate previous members
    ...
    public function joinGroup($code)
    {
        ...
        // Total members = 1 (Owner) + existing shares + 1 (new joiner)
    ...
    private function recalculateAutoSplits(Subscription $subscription): void
    {
        ...
        // Total members = 1 (Owner) + shares count
